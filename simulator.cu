...
SimulationReport simulate(std::string filename)
{
    ...
    for (int i = 0; i < decoders; i++)
    {
        ...
        /*for (int j = 0; j < codeInfo->varNodes; j++)
            std::cout << codewords[codeInfo->varNodes * i + j] << " ";
        std::cout << std::endl;*/
    }
    float* estimation;
    ...
    if (decoders > NUMBER_OF_CODEWORDS)
        decoders = NUMBER_OF_CODEWORDS;
    ...
    for (int run = 0; run < numberKernelRuns; run++)
    {
        ...
        /*for (int i = 0; i < noisedVectorSize; i++)
            std::cout << noisedVector[i] << std::endl;*/
        // Kernel execution
        /*decodeAWGN<<<decoders, block_size>>>(
                codeInfo,
                edgesFromVariable,
                edgesFromCheck,
                probP,
